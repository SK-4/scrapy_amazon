...
class CourseScraper(scrapy.Spider):
    ...
    def start_requests(self):
        return [scrapy.FormRequest('https://greycampus.knoji.com/promo-codes/', 
    ...
    def parse_konji(self, response):
        ...
        couponTable = response.xpath("//div[@class='module filter-promo']/h2[contains(text(),'GreyCampus Discount Codes: Complete Timetable')]/..")
        ...
        yield scrapy.Request('https://www.grabon.in/greycampus-coupons/', self.parse_grabon, headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64; rv:107.0) Gecko/20100101 Firefox/107.0'})
    ...
    def parse_grabon(self, response):
            ...
            if re.search('(100%|Free Trial|Scholarship)',desc):
                ...
                if re.search("All Coding Ninjas Courses|On Courses|All Online Courses",desc, re.IGNORECASE):
                    ...
                    if re.search("(Live|Instructor).*? Training", desc,re.IGNORECASE):
                        if re.search("certified|certification", desc, re.IGNORECASE):
                            availableOffers['Applicability'] = 'Course Specific'
                            availableOffers['Course Specific'] = desc.split('Starting')[0]
                        else:
                            availableOffers['Applicability'] = 'Instruction Type'
                    else:
                        if re.search("(certification|certified|course|Program)", desc, re.IGNORECASE):
                            ...
                            availableOffers['Applicability'] = 'Category'
                            availableOffers['Category'] = desc.split('Complete')[-1]
        ...
        yield self.df_coupons.to_csv("greycampus.csv",sep=",")